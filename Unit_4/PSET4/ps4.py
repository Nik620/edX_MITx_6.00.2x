# Problem Set 4: Regression in Temperatures
...
def generate_models(x, y, degs):
    ...
    polynomial = []
    for deg in degs:
        polynomial.append(np.polyfit(x,y,deg))
    return polynomial
...
def r_squared(y, estimated):
    ...
    """
    # calculate mean for the original dataset of y, NOT estimated
    mean = 0
    for val in y:
        mean += val
    mean = mean / len(y)
    # calculating the numerator and denominator to find R**2
    numerator = 0.0
    denominator = 0.0
...
def evaluate_models_on_training(x, y, models):
    ...
    # graph points & line for measurement
    for model in models:
        pylab.plot(x, y, 'bo', label = 'Temperature Data')
        a,b = models[0][0], models[0][1]
        estY = a*x + b
        pylab.plot(x, estY, 'R', label = 'Linear fit, R^2 = ' 
                    + str(r_squared(y, estY)))
    # plotting graph
